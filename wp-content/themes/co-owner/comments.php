...
<div id="comments">
	...
	?>
		...
		<p>
			<?php echo __('Comments are closed', 'co_owner')?>
		</p>
	...
	$comments_arg = array(
		...
		'fields' => apply_filters( 'comment_form_default_fields', array(
				'autor' 				=> '<div class="mb-3">' . '<label for="author">' . __( 'Name', 'co_owner' ) . '</label> ' . ( $req ? '<span>*</span>' : '' ) .
										'<input id="author" name="author" class="form-control" type="text" value="" size="30"' . $aria_req . ' />'.
										'<p id="d1" class="text-danger"></p>' . '</div>',
				'email'					=> '<div class="mb-3">' .'<label for="email">' . __( 'Email', 'co_owner' ) . '</label> ' . ( $req ? '<span>*</span>' : '' ) .
										'<input id="email" name="email" class="form-control" type="text" value="" size="30"' . $aria_req . ' />'.
										'<p id="d2" class="text-danger"></p>' . '</div>',
				'url'					=> '')),
				'comment_field'			=> '<div class="mb-3">' . '<label for="comment">' . __( 'Comment', 'co_owner' ) . '</label><span>*</span>' .
	...
	?>
		...
		<script>
			...
			*/
			...
			if (x === null || x === "") {
				d1.innerHTML = "<?php echo __('Name is required', 'co_owner'); ?>";
				flag = false;
			} else {
				d1.innerHTML = "";
			}
			...
			if (y === null || y === "") {
				d2.innerHTML = "<?php echo __('Email is required', 'co_owner'); ?>";
				flag = false;
			} else {
				d2.innerHTML = "";
			}
			...
			if (z === null || z === "") {
				d3.innerHTML = "<?php echo __('Comment is required', 'co_owner'); ?>";
				flag = false;
			} else {
				d3.innerHTML = "";
			}
