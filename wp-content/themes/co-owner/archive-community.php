<?php
	...
	*/
	...
	$community_categories= get_terms( array(
    'taxonomy' => 'assistance',
    'hide_empty' => false,
	) );	
...
$getpos = strpos($current_url,"?");
$getpos1 = strpos($current_url,"&view");
$getpos2 = strpos($current_url,"?view");
$urlgrid = str_replace("?view=list"," ",$current_url);
$urlgrid = str_replace("?view=list"," ",$current_url);
$urlgrid = str_replace("&view=list"," ",$urlgrid);
if($getpos > 0 && $getpos1 == "" ){
	
	$current_url = $current_url."&view=list";
}else{
if($getpos2 == "" && $getpos1 == ""){
$current_url = $current_url."?view=list";
}
...
<div class="center-area">
					...
					<div class="title-area pb-0">
						<h3 class="d-flex align-items-center">
							...
							<div class="list-btns ms-auto d-none d-md-block">
                                <div class="btn-group">
                                    <a href="javascript:void('0')" onclick="location.href='<?php echo $current_url;  ?>'" class="btn btn-orange list <?php if(isset($_GET['view'])){ ?>active <?php } ?>">List View</a>
                                    <a href="javascript:void('0')" onclick="location.href='<?php echo $urlgrid;  ?>'" class="btn btn-orange map <?php if(!isset($_GET['view'])){ ?>active <?php } ?>">Grid View</a>
								</div>
							</div>
						</h3>
					</div>
    ...
    <div class="main-section srt-section py-40px">
        <div class="container">
            <div class="row">
                <form action="" method="get" id="property-view">
                    <div class="col-sm-12 d-md-flex">
                        <div class="title-select med-select">
                            <select name="p_cat" class="single-select2">
                                <option value="">Category</option>
                                <?php foreach ($community_categories as $term ): ?>
								<option <?php echo $filters['p_cat'] == $term->slug ? 'selected' : null; ?>  value="<?php echo $term->slug; ?>"><?php echo $term->name; ?></option>
                                <?php endforeach; ?>
							</select>
						</div>
...
<div class="main-section mt-5 pb-5 blog-main-pg changelayout-listview">
<div class="container ">
...
<div class="blog-items-main row">
	...
	<div class="listview-blog-content">
		...
		<table>
			...
			<tbody>
					...
					$lawyer_logo = get_field('logo', get_the_ID());
					
					if(is_array($lawyer_logo)){
					$logo_id = $lawyer_logo['ID'];
					
					}else{
					$logo_id = $lawyer_logo;
					}
					$image_attributes = wp_get_attachment_image_src( $attachment_id = $logo_id );
			...
			<tr>
				...
				<td class="blog-list-details"><div class="short-des-blog"><?php  echo $short_description; ?></div></td>
				<td class="blog-list-links"><a class="blog-readmore btn btn-orange rounded-pill ms-auto min-w-120px " href="<?php esc_url( the_permalink() ); ?>" rel="bookmark"><?php echo $view_button; ?></a></td>
			</tr>
