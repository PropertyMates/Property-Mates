...
class WpdiscuzWalker extends Walker_Comment implements WpDiscuzConstants {
    ...
    public function start_el(&$output, $comment, $depth = 0, $args = [], $id = 0) {
        ...
        if ($comment->comment_parent == 0) {
			...
			$users = get_userdata($parentComment->user_id);
			...
			$parentCommentUserNames = @$users->first_name .' '.@$users->last_name;
            ...
            $replace[] = apply_filters("wpdiscuz_comment_author", $parentCommentUserNames, $parentComment);
            $showReplyTo = true;
        }
		...
		$authorAvatar = $trackOrPingback ? "<img class='avatar avatar-" . esc_attr($user["gravatarArgs"]["wpdiscuz_gravatar_size"]) . " photo' width='" . esc_attr($user["gravatarArgs"]["wpdiscuz_gravatar_size"]) . "' height='" . esc_attr($user["gravatarArgs"]["wpdiscuz_gravatar_size"]) . "' src='" . esc_url($args["avatar_trackback"]) . "' alt='trackback'>" : '<a href="/person-details/?id='.$user['gravatarUserId'].'">'.$user["avatar"].'</a>';
		}
        ...
        if ($this->options->moderation["displayEditingInfo"] && isset($commentMetas[self::META_KEY_LAST_EDITED_AT]) && isset($commentMetas[self::META_KEY_LAST_EDITED_BY])) {
			...
			$editeduser = @$lastEditUser->first_name .' '.@$lastEditUser->last_name;
            $username = $lastEditUser ? $editeduser : $comment->comment_author;
