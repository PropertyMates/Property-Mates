...
class WpdiscuzHelper implements WpDiscuzConstants {
    ...
    public function userNicename($html, $comment, $user) {
        if (apply_filters("wpdiscuz_show_nicename", $this->options->subscription["enableUserMentioning"]) && isset($user->first_name)) {
            $html .= "<span class='wpd-user-nicename'>" . esc_html($user->first_name) .' '.esc_html($user->last_name)."</span>";
        }
        return $html;
    }
        ...
        if ($this->options->login["enableProfileURLs"]) {
            if ($user["profileUrl"]) {
                ...
                if ($attributes && is_array($attributes)) {
                    ...
                    $user["authorAvatarSprintf"] = "<a rel='noreferrer ugc RRRR333' href='" . str_replace("%", "%%", $user["profileUrl"]) . "' target='_blank'>%s</a>";
                }
            }
            if ((($href = $user["commentAuthorUrl"]) && $this->options->login["websiteAsProfileUrl"]) || ($href = $user["profileUrl"])) {
                $rel = "noreferrer ugc RRR222";
