	...
	public static function uat_get_alertbox( $type, $response, $uat_key ) {
		if ( 'error' === $type ) :
			...
			if ( isset( $response['type'] ) && 'activated' === $response['type'] ) :
				$messages .= '<p>Thanks for activating the premium licence.</p><p>We recommend you to review the <a href="' . esc_url( admin_url( '/admin.php?page=moove-activity-log&tab=activity-settings' ) ) . '" class="uat_admin_link">General Settings</a> screen and adjust the settings accordingly.</p>';
			endif;
			?>
