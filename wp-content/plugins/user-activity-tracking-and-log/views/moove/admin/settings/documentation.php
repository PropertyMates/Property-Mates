		...
		<div class="uat-et-trigger-box-actions-f uat-et-help-example-f">
			<div class="et-box-content">
				...
				<div class="trigger-collapse-example trigger-collapse-example-cm" style="display: block">
					...
					<textarea id="<?php echo esc_attr( uniqid( strtotime( 'now' ) ) ); ?>"><?php echo $code; ?></textarea>
					<div class="uat-code"></div><!--  .gdpr-code -->
				</div>
				<!-- .trigger-collapse-example -->
			</div>
			<!-- .et-box-content -->
		...
		<div class="uat-et-trigger-box-actions-f uat-et-help-example-f">
			<div class="et-box-content">
				...
				<div class="trigger-collapse-example trigger-collapse-example-cm" style="display: block">
					...
					add_action('uat_csv_header','uat_csv_header_extension', 10, 1);
					function uat_csv_header_extension( $csv_header ) {
						$csv_header[] = 'User URL';
						return $csv_header;
					}


					// Extensing the data rows
					add_action('uat_csv_row','uat_csv_row_extension', 10, 2);
					function uat_csv_row_extension( $csv_row, $log_data ) {
						if ( isset( $log_data->user_id ) && intval( $log_data->user_id ) ) :
							$user = get_user_by( 'id', intval( $log_data->user_id ) );
							$csv_row['user_url'] = $user->user_url;
						else :
							$csv_row['user_url'] = '';
						endif;
						return $csv_row;
